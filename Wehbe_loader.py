...
if __name__ == '__main__':
    ...
    # Extract voxel indices corresponding to these ROIs                 #if roi_num in language_rois
    voxel_indices = [i for i, roi_num in enumerate(meta['colToROInum'][0])]
    ...
    filtered_fmri_data = fmri_time_series[:, voxel_indices]
    ...
    def preprocess_and_save(subject):
        ...
        filtered_fmri_data = fmri_data[:, voxel_indices]
        ...
        preprocessed_volume = preprocessed_volume.transpose(1, 2, 3, 0)

        # Create a new NIfTI image for the preprocessed volume
        preprocessed_volume_img = nib.Nifti1Image(preprocessed_volume, affine)

        print(f'Preprocessed data shape: {preprocessed_volume_img.shape}')

        print(preprocessed_volume_img)

        # Save the preprocessed volume
        output_file = f'{subject}_almost_preprocessed_volume.nii'
